...
텍스처를 생성할 때 (`device.createTexture`를 호출할 때)에는 'dimension' 인자 값으로 "1d", "2d", "3d" 의 세 가지 선택지만이 존재하는 점에서 혼동의 여지가 있습니다. 
기본값이 "2d" 여서 지금까지 차원(dimension)을 설정할 필요가 없었던 겁니다.
...
포맷을 이해하는 법은, 예를들어 "rg16float"의 경우 첫 부분은 텍스처에서 
...
<a id="a-depth-stencil-formats"></a>아래는 깊이(depth)와 스텐실(stencil) 포맷입니다.
