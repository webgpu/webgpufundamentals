...
export function lutTo2D3Drgba8(lut) {
  ...
  const src = lut.data;
  const data = new Uint8Array(src.length / 3 * 4);
  const offset = (offX, offY, offZ, numComponents) => {
    return (offX + offY * size + offZ * size * size) * numComponents;
  };
  for (let dz = 0; dz < size; ++dz) {
    for (let dy = 0; dy < size; ++dy) {
      for (let dx = 0; dx < size; ++dx) {
        ...
        const sy = dy;
        const sz = dz;
        const sOff = offset(sx, sy, sz, 3);
        const dOff = offset(dx, dy, dz, 4);
